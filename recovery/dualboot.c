...
static void dualboot_init_part(int num, const char* part) {
	...
	// build paths
	sprintf(path_source, "/dev/block/platform/sdhci-tegra.3/by-name/%s", part);
...
static void dualboot_prepare_env(void) {
	dualboot_init_part(PART_SYSTEM, "APP");
	dualboot_init_part(PART_SYSTEM1, "AP1");
	dualboot_init_part(PART_BOOT, "LNX");
	dualboot_init_part(PART_BOOT1, "LN1");
	dualboot_init_part(PART_MODEM, "MDM");
}
